...
void _parse_component_index(
  ...
  for (Index index : component_index) {
    ...
    convergence_check_params.emplace(SamplerComponent(function.name, index),
...
void _parse_component_name(
  ...
  for (Index name : component_name) {
    ...
    SamplerComponent first(*quantity, index);
    SamplerConvergenceParams second(precision, confidence);
...
void _parse_convergence_criteria(
  ...
  for (Index i = 0; i < it->size(); ++i) {
    ...
    // parse "confidence", "precision"
    double confidence = 0.95;
    parser.optional(confidence, option / "confidence");

    double precision;
    parser.require(precision, option / "precision");
    SamplerConvergenceParams sampler_convergence_params(precision, confidence);
